...
public class AzureRequestBodyBuilder implements AiRequestBodyBuilder {
    ...
    @Override
    public Object buildRequestBodyForAiGrader(String instruction, String textAssignment, List<String> imageUrlsAssignment, AiFeatureProperties aiFeatureProperties) {
        List<Map<String, Object>> contentList = new ArrayList<>();

        // Add text content if provided
        if (textAssignment != null && !textAssignment.isBlank()) {
            contentList.add(Map.of("type", "text", "text", textAssignment));
        }

        // Add image URLs if provided
        if (imageUrlsAssignment != null && !imageUrlsAssignment.isEmpty()) {
            for (String imageUrl : imageUrlsAssignment) {
                if (imageUrl != null && !imageUrl.isBlank()) {
                    contentList.add(Map.of("type", "image_url", "image_url", Map.of("url", imageUrl.trim())));
                }
            }
