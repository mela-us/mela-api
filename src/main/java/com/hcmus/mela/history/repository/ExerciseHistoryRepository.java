...
public interface ExerciseHistoryRepository extends MongoRepository<ExerciseHistory, UUID>, ExerciseHistoryCustomRepository {
    ...
    @Query("{ 'completedAt' : { $gte : ?0, $lt : ?1 } }")
    List<ExerciseHistory> findByCompletedAtBetween(LocalDateTime start, LocalDateTime end);

    @Aggregation(pipeline = {
            "{ $match: { 'completedAt' : { $gte : ?0, $lt : ?1 }, 'completedAt' : { $ne : null } } }",
            "{ $group: { _id: { $hour: '$completedAt' }, count: { $sum: 1 } } }",
            "{ $project: { hour: '$_id', count: 1, _id: 0 } }"
    })
    List<HourlyCount> countByHourInMonth(LocalDateTime start, LocalDateTime end);
    ...
    @Aggregation(pipeline = {
            "{ $match: { 'completedAt' : { $gte : ?0, $lt : ?1 }, 'startedAt' : { $ne : null }, 'completedAt' : { $ne : null } } }",
    ...
    List<LevelTopicCount> countByLevelAndTopic(LocalDateTime start, LocalDateTime end);
